  ...
  if (file.is_open() == false) {
    printf("Error: can't open functions.txt file.\n");
     ...
     while (getline(file, line)) {
      ...
      printf("#else\n");
      for (size_t i = 0, count = s_ParameterNames.size(); i < count; i += 1) {
        printf("typedef struct _%s_%s { _%s_%s(%s value): value(value) {}; %s value; _%s_%s(const _%s_%s &) = delete; } _%s_%s;\n", randomHex, s_ParameterNames[i].c_str(), randomHex, s_ParameterNames[i].c_str(), s_ParameterTypes[i].c_str(), s_ParameterTypes[i].c_str(), randomHex, s_ParameterNames[i].c_str(), randomHex, s_ParameterNames[i].c_str(), randomHex, s_ParameterNames[i].c_str());
      }
      ...
      for (size_t i = 0, count = s_ParameterNames.size(); i < count; i += 1) {
        ...
        printf("_%s_%s & %s", randomHex, s_ParameterNames[i].c_str(), s_ParameterNames[i].c_str());
      }
      printf(") {\n");
